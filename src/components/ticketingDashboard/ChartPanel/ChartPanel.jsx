...
class ChartPanel extends Component {
  ...
  render() {
    const { 
      ...
      percentUnassignedTickets } = this.props;
    ...
    return (
      ...
      <section>
        <article className='all-in-progess'>
            ...
            <Chart strokeDash={this.calculateStrokeDash(percentInProgressTickets)}/>
        </article>
        <article className='own-in-progress'>
            <h3>Own in Progress</h3>
            <Chart strokeDash={this.calculateStrokeDash(percentUserInProgressTickets)}/>
        </article>
        <article className="unassigned">
            <h3>Unassigned</h3>
            <Chart strokeDash={this.calculateStrokeDash(percentUnassignedTickets)}/>
        </article>
      </section>
      </div>
    );
